...
11.04\% respectively. On the Table \ref{rankEx} it is seen how often each
face shape that is provided to the algorithm returns true positives.
Even though in the ``Including'' confusion matrix, Figure \ref{coIncluding},
this overlapping from one class onto another is highly reduced. This
is seen by the presence of many near zero values is present on the table, the overlap
is still present on some specific points of the data which demonstrate a constraint of the method on differentiating similar
